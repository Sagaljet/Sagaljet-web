...
export const uploadDesignMiddleware = upload.array("images", 10);

//  Get all active (printable) designs
export const getDesigns = async (req: Request, res: any) => {
  try {
    ...
    const transformedDesigns = designs.map((design) => ({
      ...design,
      components: design.productComponents.map((pc) => ({
        id: pc.id,
...
export const getAllDesigns = async (req: projectRequest, res: Response) => {
  try {
    const designs = await prisma.design.findMany({
...
//  Get design by ID
export const getDesign = async (req: projectRequest, res: any) => {
  try {
    const { id } = req.params;
...
export const updateDesignsOrder = async (
  req: ExpressRequest<any, any, { orderedIds: string[] }>,
  res: any,
) => {
  try {
    ...
    const numericIds = orderedIds.map((id: any) => {
    ...
    await prisma.$transaction(
      numericIds.map((id: number, index: number) =>
...
export const addDesign = async (req: projectRequest, res: Response) => {
  try {
    const {
      title,
      description,
    ...
    let parsedFeatures: { name: string; value: string; extraCost: number }[] =
      [];
    if (features) {
      parsedFeatures =
    ...
    let parsedComponents: { optionId: number; extraPrice: number }[] = [];
    if (components) {
      parsedComponents =
        typeof components === "string" ? JSON.parse(components) : components;
    }

    const productExist = await prisma.design.findFirst({
      where: { slug },
    });
    ...
    const completeDesign = await prisma.design.findUnique({
      where: { id: design.id },
      include: {
        features: true,
        productComponents: {
...
//  Edit design
export const editDesign = async (req: projectRequest, res: any) => {
  try {
    const { id } = req.params;
    const {
      title,
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        try {
          if (!file.buffer) {
            ...
            console.error(
              "   Multer might be using disk storage instead of memory storage",
            );
          ...
          // Upload to R2
          const uploadedUrl = await uploadToR2(processedFile);
          ...
          if (uploadedUrl) {
            ...
            console.error(
              `   ❌ uploadToR2 returned null for ${file.originalname}`,
            );
          }
        } catch (uploadError) {
          console.error(
    ...
    if (title && title !== existingDesign.title) {
      ...
      const slugExists = await prisma.design.findFirst({
        where: {
          slug,
          id: { not: parseInt(id) },
        },
...
export const updateSingleDesignOrder = async (req: Request, res: any) => {
  try {
    const { designId, newOrder } = req.body as any;
...
//  Delete design
export const deleteDesign = async (req: projectRequest, res: Response) => {
  try {
    const { id } = req.params;
...
export const toggleDesignPrintable = async (req: projectRequest, res: any) => {
  try {
    const { id } = req.params;
    const existingDesign = await prisma.design.findUnique({
      where: { id: parseInt(id) },
    ...
    res.json({
      result: design,
      message: `Design ${
        design.isPrintable ? "made printable" : "set as non-printable"
      } successfully`,
      success: true,
    });
  } catch (error) {
...
//  Get design by slug
export const getDesignBySlug = async (req: projectRequest, res: any) => {
  try {
