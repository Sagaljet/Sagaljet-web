// src/components/OrderDesign/OrderDesignFormDialog.tsx
...
interface OrderDesignFormDialogProps {
  isOpen: boolean;
  onClose: () => void;
  editData?: OrderDesign | null;
  onSuccess: () => void;
}
...
const OrderDesignFormDialog: React.FC<OrderDesignFormDialogProps> = ({
  isOpen,
  onClose,
  editData,
  onSuccess,
  ...
  useEffect(() => {
    if (isOpen && editData) {
      ...
      if (editData.images) {
        setExistingImage(editData.images[0]);
  ...
  useEffect(() => {
    if (createSuccess) {
      toast.success(createMessage || "Order design created successfully");
  ...
  }, [createSuccess, createError, createMessage, dispatch, onSuccess]);

  // Handle update success/error
  useEffect(() => {
    if (updateSuccess) {
      toast.success(updateMessage || "Order design updated successfully");
  ...
  }, [updateSuccess, updateError, updateMessage, dispatch, onSuccess]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const submitData = {
      id: isEdit && editData ? editData.id : undefined,
  ...
  return (
    <Dialog
      // isOpen={isOpen}
      // onClose={handleClose}
      // title={isEdit ? "Edit Order Design" : "Create Order Design"}
      // size="lg"
    >
...
export default OrderDesignFormDialog;